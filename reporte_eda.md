...
Del dataset limpio de 42 atributos, la selección de los cinco siguientes se fundamentó en tres criterios simultáneos: (1) **relevancia para la hipótesis de supervisión** — la variable debe permitir identificar contratos con mayor probabilidad de desviación; (2) **capacidad de segmentación** — cardinalidad útil para discriminar sin perder poder estadístico; y (3) **controlabilidad auditora** — la variable debe ser accionable por la oficina de control interno como criterio de priorización. Se excluyeron explícitamente `nombre_entidad` por su alta cardinalidad (capturada mejor por `sector`), `departamento`/`ciudad` por carecer de hipótesis directa sobre riesgo en bienes, `duracion_dias` por ser una variable derivada más útil en análisis bivariado, y `valor_pagado`, `valor_pendiente_de_pago` y `dias_adicionados` por ser **componentes directos de la variable objetivo**, lo que introduciría data leakage conceptual en el análisis univariado independiente.
